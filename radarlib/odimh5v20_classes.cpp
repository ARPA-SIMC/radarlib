...
void OdimObject::checkMandatoryInformations()
{
	...
	if (conventions != OdimH5v20::CONVENTIONS_ODIM_H5_V2_0)
...
{
	return HDF5Attribute::getStr(group, OdimH5v20::ATTRIBUTE_ROOT_CONVENTIONS);	
}

void OdimObject::setConventions() 
{
	HDF5Attribute::set(group, OdimH5v20::ATTRIBUTE_ROOT_CONVENTIONS, OdimH5v20::CONVENTIONS_ODIM_H5_V2_0);
}
...
void OdimData::getDataDimension(int* height, int* width)
{
	...
	try
	{
		...
		if (dataset)
		{
			...
			if (numsizes != 2)
				throw OdimH5v20::OdimH5FormatException("Dataset is not a matrix");			
			*height = (int)sizes[0];
			*width  = (int)sizes[1];
		}
		else
...
{			
	...
	setObject	(OdimH5v20::OBJECT_PVOL);
	setVersion	(ModelVersion(2,0).toString());
...
void	PolarVolume::checkMandatoryInformations()
{
	...
	if (object != OdimH5v20::OBJECT_PVOL)
		throw OdimH5FormatException(std::string("OdimH5 object is not ") + OdimH5v20::OBJECT_PVOL);	

	std::string version = this->getVersion();
	if (version != ModelVersion(2,0).toString())
		throw OdimH5FormatException("OdimH5 object version is not " + ModelVersion(2,0).toString());	
...
{		
	setProduct(OdimH5v20::PRODUCT_SCAN);		
}
...
std::vector<AZAngles>	PolarScan::getAzimuthAngles	()					
{ 
	try
	{
		int			numrays = this->getNumRays();
		std::vector<AZAngles>	result	= getHow()->getAZAngles(ATTRIBUTE_HOW_AZANGLES);		
		...
		{	
			...
			{
				result[i].start = (double)i     * 360. / (double)numrays;
				result[i].stop  = (double)(i+1) * 360. / (double)numrays;
			}
			return result;
		}	
		...
		ss << "AZangles values ("<<result.size()<<") are not as many as numrays ("<<numrays<<")";
		throw OdimH5FormatException(ss.str());
	}
	catch (std::exception& e)
	{
		throw OdimH5Exception(std::string("Error getting azangles: ") + e.what());
	}
}
void			PolarScan::setAzimuthAngles	(const std::vector<AZAngles>& val, int precision)	{	 getHow()->set		(ATTRIBUTE_HOW_AZANGLES, val, precision);	}
...
void			PolarScan::setElevationAngles	(const std::vector<double>& val, int precision)	{        getHow()->set		(ATTRIBUTE_HOW_ELANGLES, val, precision);	}
std::vector<AZTimes>	PolarScan::getAzimuthTimes	()					
{ 
	try
	{
		int			numrays = this->getNumRays();
		std::vector<AZTimes>	result	= getHow()->getAZTimes	(ATTRIBUTE_HOW_AZTIMES);	
		...
		if (result.size() == 0)
		{	
			time_t stardatettime	= this->getStartDateTime();
			time_t endtdatetime	= this->getEndDateTime();
			time_t diff		= endtdatetime - stardatettime;

			result.resize(numrays);
			for (int i=0; i<numrays; i++)
			{
				result[i].start = stardatettime + (double)diff / (double)numrays * (double)i;
				result[i].stop  = stardatettime + (double)diff / (double)numrays * (double)(i+1);
			}
			return result;
		}	
		...
		/* se il numero di elementi non corrisponde c'e' qauclacosa che non va*/
		std::ostringstream ss;
		ss << "aztimes values ("<<result.size()<<") are not as many as numrays ("<<numrays<<")";
		throw OdimH5FormatException(ss.str());
	}
	catch (std::exception& e)
	{
		throw OdimH5Exception(std::string("Error getting aztimes: ") + e.what());
	}
}
void			PolarScan::setAzimuthTimes	(const std::vector<AZTimes>& val)	{        getHow()->set		(ATTRIBUTE_HOW_AZTIMES, val);	}
...
double		PolarScan::getMDS		()				{ return getHow()->getDouble	(ATTRIBUTE_HOW_MDS, 0);			}
void		PolarScan::setMDS		(double val)			{        getHow()->set		(ATTRIBUTE_HOW_MDS, val);		}
...
{			
	OdimObject::setMandatoryInformations();
//	setObject	(OdimH5v20::OBJECT_IMAGE);
	setVersion	(ModelVersion(2,0).toString());
	setDateTime	(Radar::timeutils::getUTC());
...
void	ImageObject::checkMandatoryInformations()
{
	...
	std::string object = this->getObject();
	if (object != OdimH5v20::OBJECT_IMAGE)
		throw OdimH5FormatException(std::string("OdimH5 object is not ") + OdimH5v20::OBJECT_IMAGE);	
...
{			
	OdimObject::setMandatoryInformations();
	setObject	(OdimH5v20::OBJECT_COMP);
	setVersion	(ModelVersion(2,0).toString());
	setDateTime	(Radar::timeutils::getUTC());
	setSource	(SourceInfo().setComment(""));
}
...
void	CompObject::checkMandatoryInformations()
{
	...
	std::string object = this->getObject();
	if (object != OdimH5v20::OBJECT_COMP)
		throw OdimH5FormatException(std::string("OdimH5 object is not ") + OdimH5v20::OBJECT_COMP);	
...
{			
	OdimObject::setMandatoryInformations();
	setObject	(OdimH5v20::OBJECT_XSEC);
	setVersion	(ModelVersion(2,0).toString());
	setDateTime	(Radar::timeutils::getUTC());
	setSource	(SourceInfo().setComment(""));
}
...
void	XsecObject::checkMandatoryInformations()
{
	...
	std::string object = this->getObject();
	if (object != OdimH5v20::OBJECT_XSEC)
		throw OdimH5FormatException(std::string("OdimH5 object is not ") + OdimH5v20::OBJECT_XSEC);	
...
void   XsecObject::setAngles (const std::vector<Angles>& val, int precision) {getWhere()->set(ATTRIBUTE_HOW_ANGLES, val, precision);	}
...
std::vector<AZAngles>	Product_2D::getAzimuthAngles	()					
{ 
	try
	{
		std::vector<AZAngles>	result	= getHow()->getAZAngles(ATTRIBUTE_HOW_AZANGLES);		
...
void			Product_2D::setAzimuthAngles	(const std::vector<AZAngles>& val, int precision)	{	 getHow()->set		(ATTRIBUTE_HOW_AZANGLES, val, precision);	}
...
void			Product_2D::setElevationAngles	(const std::vector<double>& val, int precision)	{        getHow()->set		(ATTRIBUTE_HOW_ELANGLES, val, precision);	}
std::vector<AZTimes>	Product_2D::getAzimuthTimes	()					
{ 
	try
	{
		std::vector<AZTimes>	result	= getHow()->getAZTimes	(ATTRIBUTE_HOW_AZTIMES);	
...
void			Product_2D::setAzimuthTimes	(const std::vector<AZTimes>& val)	{        getHow()->set		(ATTRIBUTE_HOW_AZTIMES, val);	}
...
double		Product_2D::getMDS		()				{ return getHow()->getDouble	(ATTRIBUTE_HOW_MDS, 0);			}
void		Product_2D::setMDS		(double val)			{        getHow()->set		(ATTRIBUTE_HOW_MDS, val);		}
...
void			Horizontal_Product_2D::setAngles		(const std::vector<Angles>& val, int precision) {getHow()->set(ATTRIBUTE_HOW_ANGLES, val, precision);	}

std::vector<Arotation>	Horizontal_Product_2D::getArotation		()  { return getHow()->getArotation(ATTRIBUTE_HOW_AROTATION); }
void			Horizontal_Product_2D::setArotation		(const std::vector<Arotation>& val, int precision) {getHow()->set(ATTRIBUTE_HOW_AROTATION, val, precision);	}
...
void   Product_RHI::setAngles (const std::vector<Angles>& val, int precision) {getWhere()->set(ATTRIBUTE_HOW_ANGLES, val, precision);	};
